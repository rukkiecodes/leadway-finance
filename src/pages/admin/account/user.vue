        ...
        <v-sheet rounded="lg">
          <v-row class="d-flex align-center">
            ...
            <v-col cols="12" sm="3">
              <div class="d-flex flex-column justify-center">
                <div class="d-flex ga-3 justify-start align-center blink">
                  <v-icon size="25" :color="user?.verified ? 'green' : 'red'">
                    {{ user?.verified ? 'mdi-check-circle' : 'mdi-close-circle' }}
                  </v-icon>
                  <p class="text-body-1 text-sm-h6 text-md-h5" :class="user?.verified ? 'text-green' : 'text-red'">
                    {{ user?.verified ? 'VERIFIED' : 'NOT VERIFIED' }}</p>
                </div>
            ...
            <v-col cols="12" sm="2">
              ...
              <p class="text-body-2 text-sm-body-1 text-md-h6 text-lg-h5">
                ${{ formatMoney(user?.totalBalance || 0) }}</p>
            </v-col>

            <v-col cols="12" sm="2">
              <p class="text-caption text-sm-body-2 text-md-body-1">Earning</p>
              <p class="text-body-2 text-sm-body-1 text-md-h6 text-lg-h5">
                ${{ formatMoney(user?.totalEarnings || 0) }}</p>
            </v-col>
          </v-row>
        </v-sheet>
      </v-col>
