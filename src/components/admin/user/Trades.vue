<template>
  <v-tabs-window-item value="trades">
    ...
    >
      ...
      <template v-slot:item.currency="{ item }">
        <p class="text-caption text--sm-body-2">{{ item?.currency }}</p>
      </template>

      <template v-slot:item.amount="{ item }">
        <p class="text-caption text--sm-body-2">{{ item?.amount }}</p>
      </template>

      <template v-slot:item.duration="{ item }">
        <p class="text-caption text--sm-body-2">{{ item?.duration }}</p>
      </template>

      <template v-slot:item.entryPrice="{ item }">
        <p class="text-caption text--sm-body-2">{{ item?.entryPrice }}</p>
      </template>

      <template v-slot:item.exitPrice="{ item }">
        <p class="text-caption text--sm-body-2">{{ item?.exitPrice }}</p>
      </template>

      <template v-slot:item.result="{ item }">
        <p class="text-caption text--sm-body-2" :class="item.result == 'LOSS' ? 'text-red' : 'text-green'">
          {{ item?.result }}</p>
      </template>

      <template v-slot:item.date="{ item }">
        <p class="text-caption text--sm-body-2">{{ new Date(item?.date.seconds * 1000).toDateString() }}</p>
      </template>

      <template v-slot:item.createdAt="{ item }">
        <p class="text-caption text--sm-body-2">{{ new Date(item?.createdAt.seconds * 1000).toDateString() }}</p>
      </template>
